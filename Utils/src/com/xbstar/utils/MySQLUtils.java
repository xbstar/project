...
public class MySQLUtils
{
	...
	/** 检查连接是否可用*/
	public static boolean validMySQLConnection(String hostName, int hostPort, String username, String password, String database)
	{
		try
		{
			String url = "jdbc:mysql://" + hostName + ":" + hostPort;
	...
	/** 检查MySQL是否可以正常链接*/
	public static ResponseEntity<String> checkMySQLConnection(String hostName, int hostPort, String username, String password, String database)
	{
		try
		{
	...
	{
		Connection connection = makeMySQLConnection("192.168.1.152", 3306, "root", "123456", "student");
		System.out.println(MySQLUtils.getTableInfo(connection,"stu",2));
		connection.close();
	}
	...
	public static Set<String> getTablePrimaryKey(Connection jdbcConnection, String table)
	{
		try
		{
			String database = jdbcConnection.getCatalog();
			...
			while (cursor.next())
			{
				pkSet.add(cursor.getString("COLUMN_NAME"));
			}
			cursor.close();
			return pkSet;
		} catch (Exception e)
	...
	public static Long getTableSplitCount(Connection jdbcConnection, String table, Object[] start, Object[] end)
	{
		...
		{
			StringBuilder sqlBuilder = new StringBuilder();
			Set<String> tablePrimaryKey = getTablePrimaryKey(jdbcConnection, table);
