...
public class UDLUtils
{
	...
	public static UDLType type(String udl)
	{
		if (ForbidFieldEmpty.matcher(udl).find()) return UDLType.FieldUDL;
		if (AllowFieldEmpty.matcher(udl).find()) return UDLType.TableUDL;
		if (AllowTableEmpty.matcher(udl).find()) return UDLType.DatabaseUDL;
