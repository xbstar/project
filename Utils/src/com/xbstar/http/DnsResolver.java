...
public class DnsResolver
{
	public static String resolve(String domain, String dnsServer, int dnsPort)
	{
		...
		{
			PrintUtils.printError("[XBStarHttp]不能构造请求报文:" + e.getMessage());
			return "出错了";
		}
		...
		{
			PrintUtils.printError("[XBStarHttp]无法连接DNS服务器" + dnsServer + ":" + e.getMessage());
			return "出错了";
		}
		try
		{
		...
		{
			PrintUtils.printError("[XBStarHttp]无法发送请求报文:" + e.getMessage());
			return "出错了";
		}
		byte[] responseBuffer;
		try
		...
		{
			PrintUtils.printError("[XBStarHttp]读取响应报文出错:" + e.getMessage());
			return "出错了";
		}
		try
		{
		...
		{
			PrintUtils.printError("[XBStarHttp]解析响应报文出错:" + e.getMessage());
			return "出错了";
		}
	}
	...
	private static String parseDnsResponse(byte[] response, String domain) throws Exception
	{
		...
		{
			if (HttpUtils.OnErrorPrint) PrintUtils.printWarning("[XBStarHttp]集群Cluster域名系统服务器无法解析" + domain);
			return "未部署";
		}
